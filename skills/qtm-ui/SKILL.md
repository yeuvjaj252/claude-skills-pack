---
name: qtm-ui
description: Use when designing QTM/IMS-style enterprise UI, table-first workspaces, data consoles, AG Grid pages, Project Grid card editing, operational dashboards, audit/history/log screens, inventory/asset-like modules, or reusable UI/UX structure for internal business applications.
license: UNLICENSED
...
| Summary cards | Fast status reading and optional filters | Compact, permission-aware, never decorative only |
| Filter/action bar | Search, tabs, branch/type/status, exports | Attached to data frame, not a distant sidebar |
...
- Header cells may use subtle per-cell gradient and borders, but pinned/action columns must match the same visual language.
