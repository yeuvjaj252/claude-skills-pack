...
Use this for top filter blocks like `All / Mismatch / Pending / Failed`. These blocks optimize page height; they are not marketing KPI cards.

Rules:
- Keep the strip table-adjacent and low-profile: four routine status filters fit in one row at `lg+`, `gap-2`/`gap-3`, `h-[72px]` to `h-[84px]`.
- Use only label + value for routine status filters. Remove captions, icons, and decorative blur unless the user explicitly asks for them.
- Active state: soft lavender surface, Dark Blue border/ring, not a heavy filled CTA.
- If search/source/action controls are needed, put a compact `QtmFilterBar` below the strip or use `QtmControlRail` only when a top toolbar would become taller than the first table rows.

```tsx
type StatusCard = { label: string; value: string; count: number };

function CompactStatusStrip({ cards, activeValue, onChange }: {
  cards: StatusCard[];
  activeValue: string;
      ...
      {cards.map((card) => {
        ...
        return (
          <button
            ...
            className={`h-[76px] rounded-[8px] border px-3 py-2.5 text-left shadow-sm transition-colors ${
              active
                ? 'border-[#010120]/25 bg-[#f6f4ff] ring-1 ring-[#010120]/10'
                : 'border-black/[0.08] bg-white hover:bg-black/[0.02]'
            }`}
          >
            <div className="font-mono text-[10px] font-medium uppercase tracking-[0.12em] text-black/45">
              {card.label}
            </div>
            <div className="mt-2 text-[26px] font-medium leading-none tracking-[-0.04em] text-[#010120]">
              {card.count}
            </div>
          </button>
        );
...
| Symptom | Cause |
|---------|-------|
| Filter cards look like a dashboard and push the grid down | Captions, icons, blur, large padding, or tall KPI anatomy were used for simple status filters |
| Users cannot tell the selected status | Active card has no lavender surface and Dark Blue border/ring |
| Controls compete with the table | Filter strip is separated from the data frame by large vertical gaps or a second unrelated panel |
...
The QTM AG Grid header standard is the `/qtm-ui-example` scoped gradient. Keep global `.ag-theme-quartz` neutral, then opt QTM modules into this per-wrapper treatment.
...
Pinned/action columns use the same header cell styling. If the pinned header looks flat or tinted differently, the scoped selector is too narrow.
